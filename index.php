...
if (isset($username) && !empty($username)){
    ...
    $PAGE->set_heading(get_string('hellouser', 'local_helloworld', $worldname));
}

echo $OUTPUT->header();

if (isset($username) && !empty($username)) {
    echo "<h1>". get_string('hellouser', 'local_helloworld', $username) ."</h1>";
    ...
    echo '<h1>' . get_string('hellouser', 'local_helloworld', $worldname) . '</h1>';
    ...
    echo '   <input type="text" name="username" placeholder="' . s($promptname) . '">';
    echo '   <input type="submit" value="' . get_string('submit', 'core_moodle') . '">';
    echo '</form>';
}
